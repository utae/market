...
import kr.co.t_woori.market.home.HomeAPIService;
...
public class BannerImageUploader extends AppCompatActivity {
    ...
    private void readyToUpload(String bannerName, List<String> imgList) {

        List<MultipartBody.Part> fileList = new ArrayList<>();

        RequestBody barcodeBody = RequestBody.create(MediaType.parse("text/plain"), bannerName);
        ...
        for(String img : imgList){
            ...
            MultipartBody.Part body = MultipartBody.Part.createFormData(bannerName + "_img", file.getName(), requestFile);
    ...
    private void uploadGoodsImg(RequestBody barcodeBody, List<MultipartBody.Part> fileList){
        APICreator.create(HomeAPIService.class).uploadBannerImg(barcodeBody, fileList).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                Utilities.showToast(BannerImageUploader.this, "업로드 성공");
                finish();
            }

            @Override
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                Utilities.showToast(BannerImageUploader.this, "업로드 실패");
                finish();
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == PhotoPicker.REQUEST_CODE && data != null) {
                ...
                readyToUpload(bannerName, imgList);
            }
        }else{
            finish();
        }
